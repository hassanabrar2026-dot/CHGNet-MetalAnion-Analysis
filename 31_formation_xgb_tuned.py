...
print("Tuning hyperparameters for formation_energy_per_atom...")
...
# Save best params and predictions
params_out = os.path.join(data_dir, "formation_xgb_best_params.json")
with open(params_out, "w") as f:
    json.dump(search.best_params_, f, indent=2)
print("Saved best params to:", params_out)

pred_out = os.path.join(data_dir, "formation_xgb_test_predictions.csv")
...
plt.tight_layout()
parity_fig = os.path.join(fig_dir, "formation_parity.png")
...
plt.scatter(y_pred, residuals, s=10, alpha=0.6, c="tab:red")
plt.axhline(0, color="k", linestyle="--", linewidth=1)
plt.xlabel("Predicted formation_energy_per_atom (eV/atom)")
plt.ylabel("Residual (pred - true) (eV/atom)")
plt.tight_layout()
res_fig = os.path.join(fig_dir, "formation_residuals.png")
plt.savefig(res_fig, dpi=1200)
plt.close()
print("Saved:", res_fig)
